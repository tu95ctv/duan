...
#     lambda val, needdata: self.env['stock.location'].search([('name','=','LTK Dự Phòng')]).id
        department_id = self.env['hr.department'].search[('name','=','LTK')].id
    ...
    for r in self:
            ...
            LOOK_DICT = {
                ...
                'sheet_names':[u'Chuyển Mạch (IMS, Di Động)'],#xl_workbook.sheet_names(),#[u'Truyền dẫn'],#[u'IP (VN2, VNP)'],[u'Chuyển Mạch (IMS, Di Động)']
...
('product_id',{'key':True,'required':True,
               'fields':[
                        ...
                        ('uom_id',  { 'func':uom_id_,'fields': [ ('name',{'xl_title':u'Đơn vị tính' ,'key':True,'required':True}),#'set_val':u'Cái',
...
('product_qty', {'func':lambda val, n: 1 if  (n['value_fields_of_instance_dicts']['prod_lot_id']['val'] and val > 1) else val ,'xl_title':u'Tồn kho cuối kỳ','key':False,'required':True }),
                         ]
